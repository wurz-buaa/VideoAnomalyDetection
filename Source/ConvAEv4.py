...
#config = tf.ConfigProto(allow_soft_placement=True, gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.7, allow_growth = True), device_count = {'GPU': 0})
...
class ConvAEv4():
    ...
    def fit(self, data, data_org, datasize):
        ...
        num_batches = int(datasize / self.batch_size)
        ...
        with tf.Session(config=config) as sess:
            ...
            while (self.epoch < self.num_epochs) and (not self.stop_training):
                ...
                for batch_idx, (batch_start, batch_end) in enumerate(batches):
                    ...
                    f = h5py.File(data, 'r')
                    x_batch = f['data'][batch_start:batch_end, :, :, :]
                    #print('Load x_batch shape:', x_batch.shape)
                    f = h5py.File(data_org, 'r')
                    x_batch_org = f['data'][batch_start:batch_end, :, :, :]
                    #print('Load x_batch_org shape:', x_batch_org.shape)
                    f.close()
