...
Examples = collections.namedtuple("Examples", "paths, inputs, targets, mask, count, steps_per_epoch")
...
def create_model(inputs, targets, mask=None, train_flag=0):
    ...
    with tf.name_scope("real_discriminator"):
        with tf.variable_scope("discriminator"):
            ...
            if train_flag > 0:
                predict_real = create_discriminator(tf.multiply(inputs, mask), tf.multiply(targets, mask))
            else:
    ...
    with tf.name_scope("fake_discriminator"):
        with tf.variable_scope("discriminator", reuse=True):
            # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
            if train_flag > 0:
                predict_fake = create_discriminator(tf.multiply(inputs, mask), tf.multiply(outputs, mask))
            else:
    ...
    with tf.name_scope("generator_loss"):
        ...
        if train_flag > 0:
            gen_loss_L1 = tf.reduce_mean(tf.abs(tf.multiply(targets - outputs, mask)))
        else:
            gen_loss_L1 = tf.reduce_mean(tf.abs(targets - outputs))
...
def pix2pix_func(mode, A, B, mask_file, data_shape, data_folder, output_folder, num_epochs, direction,
    ...
    steps_per_epoch = int(math.ceil(data_shape[0] / a.batch_size))
    ...
    inputs_batch = tf.placeholder(tf.float32, shape=[1, data_shape[1], data_shape[2], data_shape[3]])
    targets_batch = tf.placeholder(tf.float32, shape=[1, data_shape[1], data_shape[2], data_shape[3]])
    mask_batch = tf.placeholder(tf.float32, shape=[1, data_shape[1], data_shape[2], 1])
    ...
    if train_flag > 0:
        model = create_model(examples.inputs, examples.targets, examples.mask, train_flag)
    else:
        model = create_model(examples.inputs, examples.targets)
    ...
    gan0_folder_name = 'hvad-gan-layer0-v5-brox'
    cae_folder_name = 'hvad-32-16-8-release'
    ...
    if layers[0] == 0:
        gan_model_folder = '%s/%s/layer%d-usecluster0-%s-large-v2-reshape' % (model_folder, gan0_folder_name, layers[0], mode_explain)

    else:
        gan_model_folder = '%s/%s/layer%d-usecluster0-%s-large-v2-reshape' % (model_folder, cae_folder_name, layers[0], mode_explain)
    ...
    #config = tf.ConfigProto(allow_soft_placement=True, gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.9, allow_growth = True), device_count = {'GPU': 0})
    ...
    with sv.managed_session(config = config) as sess:
        ...
        if train_flag == 1:
            a.checkpoint = gan_model_folder

        if a.checkpoint is not None:
            print("loading model from checkpoint %s" % gan_model_folder)
            checkpoint = tf.train.latest_checkpoint(a.checkpoint)
        ...
        if a.mode == "test":
            ...
            start = time.time()
            n = 8
            load_size = int(data_shape[0] / n)
            #times = 0
            #load_idx = 0
            cur_epoch = 0
            for step in range(max_steps):

                img_idx = step % data_shape[0]
                ...
                load_idx = load_idx % load_size
                if times == n and data_shape[0] % n > 0 and load_idx == 0:
                    f = h5py.File(A, 'r')
                    data_F = f['data'][load_size*times:load_size*times + data_shape[0] % n]
                    print('Load data_F shape:', data_F.shape)
                    f = h5py.File(B, 'r')
                    data_M = f['data'][load_size*times:load_size*times + data_shape[0] % n]
                    print('Load data_M shape:', data_M.shape)
                    if train_flag > 0:
                        f = h5py.File(mask_file, 'r')
                        data_mask = f['data'][load_size*times:load_size*times + data_shape[0] % n]
                        print('Load data_mask shape:', data_mask.shape)
                    f.close()
                    
                
                if times < n and load_idx == 0:
                    f = h5py.File(A, 'r')
                    data_F = f['data'][load_size*times:load_size*times + load_size]
                    print('Load data_F shape:', data_F.shape)
                    f = h5py.File(B, 'r')
                    data_M = f['data'][load_size*times:load_size*times + load_size]
                    print('Load data_M shape:', data_M.shape)
                    if train_flag > 0:
                        f = h5py.File(mask_file, 'r')
                        data_mask = f['data'][load_size*times:load_size*times + load_size]
                        print('Load data_mask shape:', data_mask.shape)
                    f.close()
                ...
                [paths1, inputs1, targets1] = sess.run([examples.paths, examples.inputs, examples.targets],
                    feed_dict={paths_batch: "%s" % step,inputs_batch: np.reshape(data_F[load_idx, :, :, :],
                                                        [1, data_F.shape[1], data_F.shape[2], data_F.shape[3]]),
                               targets_batch: np.reshape(data_M[load_idx, :, :, :],
                                                         [1, data_F.shape[1], data_F.shape[2], data_F.shape[3]])})
                ...
                if train_flag > 0:
                    results = sess.run(fetches, options=options, run_metadata=run_metadata,
                                        feed_dict = {    paths_batch: "%s" % step,
                            inputs_batch: np.reshape(data_F[load_idx, :, :, :], [1, data_F.shape[1], data_F.shape[2], data_F.shape[3]]),
                            targets_batch: np.reshape(data_M[load_idx, :, :, :], [1, data_F.shape[1], data_F.shape[2], data_F.shape[3]]),
                            mask_batch: np.reshape(data_mask[load_idx, :, :, :], [1, data_F.shape[1], data_F.shape[2], 1])})
                else:
                    results = sess.run(fetches, options=options, run_metadata=run_metadata,
                                        feed_dict = {    paths_batch: "%s" % step,
                            inputs_batch: np.reshape(data_F[load_idx, :, :, :], [1, data_F.shape[1], data_F.shape[2], data_F.shape[3]]),
                ...
                '''
                if img_idx == data_shape[0] - 1:
                    print("saving model")
                    modelname = "model_epoch%d" % (cur_epoch)
                    saver.save(sess, os.path.join(a.output_dir, modelname), global_step=sv.global_step)
                if sv.should_stop():
                    break
